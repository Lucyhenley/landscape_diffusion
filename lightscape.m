...
global roost acceptance_rate  box sample_size base_D N T dt range n 
roost = [274257, 66207];
...
light_hits = zeros(sample_size, 1);

tic

[alpha, threshold, D] = generate_diffusion_parameters(data, prior)

%%

parfor j = 1:sample_size    
    [alpha(j), threshold(j), D] = generate_diffusion_parameters(roads, 0);
    
  %  alpha(j) = 5*rand - 2;
  %  percentile = rand();
  %  threshold(j) = prctile(nonzeros(data), percentile);
  ...
  %  data(data > threshold) = 1;
   ...
   % D = base_D*exp(alpha*data);
    ...
    [x,y, light_hits(j)] = landscape_dependent_diffusion(D);
